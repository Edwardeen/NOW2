...
// Define Transaction interface locally if not imported from elsewhere
interface User {
    id: number;
    email: string;
    address: string;
...
interface Transaction {
  ...
  UserID: number;
  User?: User | null; // Allow User to be optional
  EntityID: number | null;
  Entity?: Entity | null; // Allow Entity to be optional
...
const CardSkeleton = () => (
  <div className="rounded-lg md:rounded-xl bg-gray-300/50 w-full p-3 sm:p-4 shadow-md animate-pulse">
    ...
    <div className="h-5 bg-gray-400 rounded w-full mb-4"></div>
...
export default function Home() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      setError(null);
      try {
         // TODO: Ideally, fetch only relevant transactions from the API
  ...
  return (
    // Changed background to bg-Cream
    <div className="flex flex-col min-h-screen bg-Cream p-4 sm:p-6 lg:p-8">
      ...
      <div className='flex-grow w-full max-w-7xl mx-auto flex flex-col gap-4 md:gap-6'>
         {/* Responsive Title */}
        <h1 className='text-Tertiary font-extrabold text-xl sm:text-2xl md:text-3xl text-center md:text-left'>Screener Dashboard: Pending Screening</h1>
        ...
        {isLoading ? (
            ...
            <div className="text-center text-red-500 bg-red-100 p-4 rounded-lg">
             ...
             <div className="text-center text-gray-500 bg-Cream p-6 rounded-lg shadow-md col-span-full"> {/* Added col-span-full */}
                 {/* Clarified Message */}
                 {data.length > 0 ? 
                    "All transactions have been screened." : 
