...
const MyForm = () => {
    ...
    const handleSubmit = async (event: React.FormEvent<HTMLFormElement>) => {
        ...
        console.log('Logging in as Screener with Username:', username);
        ...
        try {
            const result = await signIn('credentials', {
                ...
                username: username,
                password: password,
                userType: 'screener', // Explicitly pass userType
            });

            if (result?.error) {
                // Handle errors returned from the authorize function
                console.error("Screener Login Error:", result.error);
                setError(result.error === "CredentialsSignin" ? "Invalid username or password." : "Login failed. Please try again.");
                setIsLoading(false);
            } else if (result?.ok) {
                // Login successful
                console.log('Screener login successful, redirecting...');
                // Redirect to the screener dashboard
                router.push('/screener/home');
        ...
        <div className="bg-Green min-h-screen flex items-center justify-center p-4">
            ...
            <div id="cardBg" className="flex flex-col w-full max-w-md h-auto items-center gap-4 sm:gap-6 px-6 py-8 sm:px-8 sm:py-10 bg-Cream rounded-lg shadow-lg">
                <div className="flex flex-col items-center text-center">
                    ...
                    />
                     {/* Responsive Text */}
                    <h1 className="text-2xl sm:text-3xl font-bold text-Tertiary">Screener Login</h1>
                    <p className="text-base sm:text-lg text-Tertiary mb-4 sm:mb-6">Access your Screener dashboard</p>
                </div>
                <MyForm />
